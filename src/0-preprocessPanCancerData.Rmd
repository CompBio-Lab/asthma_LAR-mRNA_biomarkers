---
title: "preprocess PanCancer nanostring data"
...
1. Import NanoString nCounter data  
2. Perform quality control assessment based on NanoString Data Analysis Guidelines  
3. Normalize and filter expression data  
...
source(here::here("src", "utils", "rcc_to_dat.R"))
...
## 1a. Read RCC files and merge datasets
...
## 1b. Fix gene symbols and export cleaned raw data
...
## 1c. Sample mapping and demographics

```{r sample_mapping_and_demographics}
...
posNF <- posDat %>% group_by(Sample) %>% 
  dplyr::summarise(posGeoMean = geometric.mean(Count), fileName = unique(fileName)) %>% 
  mutate(posNF = mean(posGeoMean)/posGeoMean)
range(posNF$posNF)
...
## LST_026
samples <- as.character(demoPre$MST_LST_numbers)[rownames(demoPre) %in% rownames(expDat)]
names(samples) <- as.character(rownames(demoPre)[rownames(demoPre) %in% rownames(expDat)])
Y <- as.data.frame(log2(t(expDat[names(samples[grep("L_ST_026", samples)]),])))
colnames(Y) <- paste(colnames(Y), as.character(demoPre[colnames(Y), "AIC_YMD"]), sep=".")
...
## MA plot
diff <- Y[, "E14.2010-03-09"] - Y[, "E17.2010-03-24"]
avg <- (Y[, "E14.2010-03-09"] + Y[, "E17.2010-03-24"])/2
...
## 3f. log2 transform total Norm data
