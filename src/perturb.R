...
scperturb <- NormalizeData(scperturb,
                           normalization.method = "LogNormalize") %>% 
  FindVariableFeatures(selection.method = "vst", 
                       nfeatures = 2000) %>% 
  ScaleData(features = rownames(scperturb)) %>% 
  RunPCA() %>% 
  RunUMAP(dims = 1:50, reduction.name = 'umap.rna', 
          reduction.key = 'rnaUMAP_')
Idents(scperturb) <- scperturb@meta.data$cluster

DimPlot(scperturb, reduction = "umap.rna", label = TRUE, 
...
scperturb <- SCTransform(scperturb)
...
sc_drugde = function(sce){
  ...
  res <- lapply(cells, function(cc){
    ...
    fit <- eBayes(lmFit(vobj, design, block = block, correlation = dupcor$consensus))
    top_all <- lapply(colnames(design)[2:144], function(i){
      ...
      do.call(rbind, .)
    list(top_all=top_all, normE = vobj$E)
  }) %>% 
    zip_nPure()
...
result <- sc_drugde(sce)
logfc <- result$toptables %>% 
  dplyr::select(logFC, gene, sm_name, cc) %>% 
  spread(gene, logFC, fill = 0)

row_ha <- rowAnnotation(cell=logfc$cc, drug=logfc$sm_name)
col_ha <- columnAnnotation(cell=logfc$cc, drug=logfc$sm_name)
Heatmap(logfc[, -c(1, 2)], right_annotation = row_ha)

Heatmap(logfc[, -c(1, 2)], right_annotation = row_ha)

Heatmap(cov(t(logfc[,-c(1,2)])),
